---
title: "GolfForecast - Preliminary analysis"
...
auxiliary <- function(golfer){
  ...
  for(week in unique(data$Week[data$GolferID==golfer])){
    result <- rbind(result, c(golfer,week,nrow(data[data$GolferID==golfer & data$Week==week,])))
  }
...
cat("There are", sum(competitions_per_golfer_week$Frequency>1),"cases of golfers with >1 competition per week and these are all the cases:")
