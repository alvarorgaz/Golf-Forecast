...
for(lag in seq(1, max_lag, 5)){
  data_preprocessed[,paste0(target,"_Lag",lag,"to",lag+4)] <- apply(data_preprocessed[,lags_names[lag:(lag+4)]], 1, auxiliary_mean)
}
```

Distribution of number of missing values in lagged features:
```{r}
missings <- sapply(1:nrow(data_preprocessed), function(x) sum(-9999==data_preprocessed[x,lags_names]))
round(table(missings)/length(missings)*100, 2)
```

## Save preprocessed data
```{r}
save(data_preprocessed, file="Data_preprocessed.RData")
```